...
const FreebiesPage = () => {
    ...
    const loadItems = async () => {
        ...
        setError(null);
        try {
            const data = await fetchCatalogue('freebie');
            setItems(data);
        } catch (err) {
            console.error('[FreebiesPage] Failed to load freebies:', err);
            setError(t('failedToLoadFreebies'));
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        ...
        >
            <SEO
                title="Free Resources | Mr. Zayn"
                description="Free English learning resources — PDF worksheets, vocabulary packs, and more. Completely free for all learners."
                url="https://myenglish.my.id/hello/freebies"
                type="website"
            />
            ...
            <section className="bg-gradient-to-br from-slate-800 via-slate-900 to-[#2a0a0a] px-6 py-12">
                <div className="max-w-4xl mx-auto">
                    ...
                    <h1 className="text-3xl md:text-4xl text-white leading-tight">
                        <span className="font-extralight block">{t('freeResources').split(' ')[0]}</span>
                        <span className="font-bold text-[#ff6b6b]">
                            {t('freeResources').split(' ').slice(1).join(' ') || t('freeResources')}
                        </span>
                    </h1>
