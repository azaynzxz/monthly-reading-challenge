            ...
            >
                ...
                <div className={`relative z-10 p-4 md:p-6 lg:p-8 ${wikiImage ? 'pointer-events-none' : 'bg-[#880000]/5 border-b border-slate-100'} ${wikiImage ? 'absolute top-0 left-0 right-0' : ''}`}>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 md:gap-3 mb-3 md:mb-4 relative">
                        {/* Metadata */}
                        <div className={`flex items-center justify-between sm:justify-start gap-2 font-bold text-xs md:text-sm uppercase tracking-wide w-full sm:w-auto pointer-events-auto ${wikiImage ? 'text-white' : 'text-[#880000]'}`}>
                            <button
                                onClick={(e) => { e.stopPropagation(); onOpenMonthSelector?.(); }}
                                className={`flex items-center gap-1 px-2 md:px-2.5 py-0.5 md:py-1 rounded-full text-xs md:text-sm transition-all hover:scale-105 active:scale-95 cursor-pointer z-20 ${wikiImage ? 'bg-white/20 backdrop-blur-sm hover:bg-white/30' : 'bg-[#880000]/10 hover:bg-[#880000]/20'}`}
                            >
                                <span>Month {currentMonth} - Day {activeData.day}</span>
            ...
            <div className="p-6 md:p-10">
                <div className="mb-4">
                    <div className="flex items-center justify-between mb-4">
                        <span className="block text-slate-400 text-xs font-bold uppercase tracking-widest">Read Aloud</span>
                        {focusedChunk !== null && (
                            <button
                                onClick={() => setFocusedChunk(null)}
                                className="text-xs text-slate-500 hover:text-[#880000] font-medium transition-colors"
                            >
                                Show All
                            </button>
                        )}
                            ...
                            return chunks.map((chunk, chunkIndex) => {
                                ...
                                return (
                                    <p
                                        ...
                                        className={`text-lg md:text-xl leading-[1.8] md:leading-[1.9] text-slate-700 font-medium cursor-pointer rounded-lg p-3 -mx-3 transition-all duration-300 ${
                                            isFocused 
                                                ? 'opacity-100' 
                                                : 'opacity-30 hover:opacity-50'
                                        } ${focusedChunk === chunkIndex ? 'bg-amber-50/50 border-l-4 border-amber-400 pl-4' : 'hover:bg-slate-50'}`}
                                    >
